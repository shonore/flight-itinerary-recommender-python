...
def hello(event, context):
    try:
        ...
        destinations = tf.data.Dataset.from_tensor_slices(dict(pandasDF))
        origins = tf.data.Dataset.from_tensor_slices(dict(pandasDF))
        ...
        user_ids_vocabulary.adapt(destinations.map(lambda x: x["id"]))
        ...
        task = tfrs.tasks.Retrieval(metrics=tfrs.metrics.FactorizedTopK(
            origins.batch(128).map(origin_model)
        ))
        ...
        # # Train for 3 epochs.
        model.fit(destinations.batch(4096), epochs=3)
